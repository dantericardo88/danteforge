...
- ✅ **#3 (code)** grok reserved as the JUDGE-ONLY third member; Claude + codex build; builder-excluded court gets ≥2 independent judges of 3 model families (commit 00bff18). ⚠ ACTIVATION: grok-build CLI must be installed + authenticated to actually convene — the live probe on this machine currently shows only 2 members (codex + claude), so the independent court is code-ready but dormant until grok is reliably available.
