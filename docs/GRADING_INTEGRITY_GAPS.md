...
- ✅ **#3 (code, BOTH halves)** grok reserved as the JUDGE-ONLY third member (commit 00bff18) AND the sequential push now excludes ALL contributing builders, not just one (commit e58c664) — closing the multi-builder self-judge hole where council-crusade built with codex+claude and then let both judge their own work. The court takes `excludeMemberIds` (full builder roster) → judge pool = judge-only members only; <2 → honest throw, never a self-certified PASS. Parallel path: single builder excluded + grok = 2 independent judges of 3 model families. ⚠ ACTIVATION: grok-build CLI must be installed + authenticated to actually convene — the live probe on this machine currently shows only 2 members (codex + claude), so the independent court is code-ready but DORMANT until grok is reliably available. ⚠ REDUNDANCY (filed as a challenge): with only ONE judge-only member, the court has zero redundancy — grok down/conflicted ⇒ no independent court at all; a robust court wants ≥2 judge-only members.
