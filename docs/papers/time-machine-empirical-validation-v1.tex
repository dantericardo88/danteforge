...
\begin{enumerate}
\item \textbf{Proof spine} (\texttt{@danteforge/evidence-chain v1.0.0}) --- every artifact carries a SHA-256 payload hash, a Merkle root, and an optional \texttt{prevHash} chain.
...
Real-fs benchmark numbers (Pass~23 override run):

\begin{tabular}{lrrrl}
\toprule
Tier & verify (ms) & restore (ms) & query (ms) & Threshold? \\
\midrule
10K  & 4{,}555  & 2 & 1{,}570 & yes \\
100K & 248{,}150 & 7 & 61{,}182 & no (verify/query above default) \\
1M   & --- & --- & --- & gated GATE-3 \\
\bottomrule
\end{tabular}

Restore scales near-linearly ($2 \to 7$ ms across $10\times$). Verify and query grow faster than linear and exceed the default PRD threshold at 100K --- a v2 optimization target. Restore stays bounded by the target commit's blob set.
...
\item Live LLM round-trip is gated; D1, D3, D4 are placeholders. Realistic budget envelope is \$10--160 depending on resolved Sonnet SKU; the reproducibility appendix pins a specific model.
\item G2 Dojo integration is out-of-scope for v1.
\item F 100K was executed under env-var override during Pass~23 remediation; verify ($\sim$248 s) and query ($\sim$61 s) exceed the default PRD threshold at the 100K tier, while restore (7 ms) stays fast. v2 optimization target. 1M run remains gated behind GATE-3.
\item Section~5.1's "0 disagreements across 100 verifications" measures verifier determinism on a single chain, not a statistical false-positive rate over the chain-construction distribution.
\item Class G is computed by side-scripts (\texttt{build-g1-substrate-validation.mjs}, \texttt{build-g4-truth-loop-ledger.mjs}) plus the Pass~21 receipt, not by \texttt{runClassG}.
...
Live DELEGATE-52 run; F 1M scale benchmark; withheld-environments coverage; alternative benchmarks; substrate-cost optimization; pre-edit interception hook (currently harness-dependent).
