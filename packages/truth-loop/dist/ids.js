...
export function nextRunId(rootDir, now = new Date()) {
    ...
    if (existsSync(truthLoopDir)) {
        for (const entry of readdirSync(truthLoopDir)) {
            if (!entry.startsWith(datePrefix))
                continue;
...
export function newVerdictId(runId) {
    return `vrd_${runId.slice(4)}`;
}
export function newNextActionId(runId) {
    return `nax_${runId.slice(4)}`;
}
export function newBudgetEnvelopeId(runId) {
