...
// Prompt builder
// ---------------------------------------------------------------------------
function buildPredictionPrompt(request) {
...
function parsePredictionResponse(raw, action, config) {
    ...
    try {
        ...
        for (const [dim, val] of Object.entries(parsed.scoreImpact ?? {})) {
            if (typeof val === 'number' && isFinite(val)) {
                scoreImpact[dim] = Math.max(-2, Math.min(2, val));
            }
        }
        ...
        const coveredDimensions = Array.isArray(parsed.coveredDimensions)
            ? parsed.coveredDimensions.filter((d) => typeof d === 'string')
            : Object.keys(scoreImpact);
        return {
            predicted: {
                scoreImpact,
...
function makeFallback(action, version, reason) {
    return {
        predicted: {
            scoreImpact: {},
            costUsd: 0.05,
 ...
 */
export async function predict(request, llmCaller, config = DEFAULT_PREDICTOR_CONFIG) {
    if (config.disabled) {
