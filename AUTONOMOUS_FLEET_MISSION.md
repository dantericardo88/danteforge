...
- 2026-06-09 | DanteSecurity | dim011_memory_injection_detection | 0.0 -> 7.0 | de-duplicated the RWX-anonymous-region detection heuristic (copied in scan_proc_maps AND run_corpus_test, drift risk) into one wired `is_injection_region()`; added 3 seam-free tests exercising the WIRED detector (DS commit 97793eb). tier T4. DanteSecurity is the lowest fleet project (derived 0.59). TWO KEY FINDINGS for the operator: (1) the Python capability tests (scripts/capability_tests_*.py) are SELF-FULFILLING STUBS — they check their own inline fixtures, never the Rust code — so earning a dim off `dante.py test <name>` passing would be FABRICATION; must earn off the real wired Rust path instead. (2) Several Rust modules are exported-but-ORPHAN (e.g. sbom.rs: wired in lib.rs but NO live consumer in main.rs) — earning those needs them genuinely wired into the live agent first (real engineering), not just a test pointer. (3) DanteForge GATE GAP noticed: isTestSuiteCommand (TEST_RUNNER_RE) only matches JS runners, so `cargo test`/`pytest`/`go test` escape the T4/7.0 test-suite cap and could be over-credited to 8.0 as runtime-exec — declared tier T4 binds it honestly here, but the gate should recognize polyglot test runners.
- 2026-06-09 | DanteSecurity | dim012_usb_and_peripheral_control | 0.0 -> 7.0 | usb_control was an ORPHAN — main.rs only did a token `let _usb_controller = UsbController::new(None)`, so evaluate_device was never reachable from the live product. GENUINELY wired it: main now runs `run_policy_test()` as a boot-time power-on self-test of the deny-default policy engine (exercises evaluate_device across block/allowlist/exception, logs PASS/FAIL = observable artifact). Added 4 seam-free `#[cfg(test)]` tests against the real UsbController (deny-default block + INV-11 education string, allowlist allow, unexpired-vs-expired exception INV-10, production self-test passes). tier T4, callsite src/endpoint/src/usb_control.rs (DS commit 7a482c0, no push). Gate-confirmed 0.0->7.0. DanteSecurity earners 12->13, derived mean 0.63 -> 0.68. Same honest shape as the dim011 cycle: a real wired-but-untested Rust module, exercised seam-free, no fabrication, NOT earned off the self-fulfilling Python capability stubs.
- 2026-06-09 | DanteSecurity | dim013_snapshot_and_backup_integrity | 0.0 -> 7.0 | snapshot.rs (SnapshotManager) was genuinely wired (main.rs calls snapshot_mgr.take_snapshot(root, 10_000) at boot) but had ZERO tests. Added 5 seam-free `#[cfg(test)]` tests driving the real SnapshotManager with real temp dirs: clean snapshot verifies clean + stable Merkle root (INV-12), modified file detected as tampering (INV-03/11), deletion+addition both surface in the delta, persisted manifest reloads byte-identically, production run_integrity_test() self-test detects tamper. tier T4, callsite src/endpoint/src/snapshot.rs (DS commit 8004958 on branch danteforge/honest-climb-DanteSecurity-20260609-01, no push). Gate-confirmed 0.0->7.0, no integrity cap. NOTE: this cycle ran AFTER the stale-evidence-collapse fix shipped (DanteForge 84fa3f1) — gap/validate now read true scores across commits. Same wired-but-untested Rust pattern as dim011/dim012.
