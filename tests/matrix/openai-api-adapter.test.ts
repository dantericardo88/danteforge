// Phase 14d — OpenAIAPIAdapter tests (mocked LLM)
//
// Tests the OpenAI API-backed adapter. The subprocess-based CodexAdapter
// has its own test file in codex-adapter.test.ts.
import { describe, it, after } from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs/promises';
import os from 'node:os';
import path from 'node:path';
import { OpenAIAPIAdapter } from '../../src/matrix/adapters/openai-api-adapter.js';
import {
  parseEdits,
...
describe('OpenAIAPIAdapter.parseEdits', () => {
...
describe('OpenAIAPIAdapter.validateEditsAgainstLease', () => {
...
describe('OpenAIAPIAdapter.buildCodingPrompt', () => {
...
// ── OpenAIAPIAdapter (with injected LLM) ───────────────────────────────────────

describe('OpenAIAPIAdapter', () => {
  it('isAvailable returns true', async () => {
    const adapter = new OpenAIAPIAdapter({ workPacket: fakePacket() });
...
describe('OpenAIAPIAdapter.collectContextFiles', () => {
