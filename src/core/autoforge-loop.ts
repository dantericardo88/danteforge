...
// State enum + public type surface + loop-result writer live in autoforge-loop-core.ts (file-size
// split). Imported for internal use AND re-exported so every existing importer/test path is unchanged.
import {
  AutoforgeLoopState, writeLoopResult, getLoopResultPath,
  type AutoforgeLoopContext, type AutoforgeLoopDeps, type AutoforgeGuidance,
  type BlockingIssue, type LoopResult, type LoopTerminationReason,
} from './autoforge-loop-core.js';
export {
  AutoforgeLoopState, getLoopResultPath, writeLoopResult,
} from './autoforge-loop-core.js';
export type {
  AutoforgeLoopContext, AutoforgeLoopDeps, AutoforgeGuidance,
  BlockingIssue, LoopResult, LoopTerminationReason,
} from './autoforge-loop-core.js';
import { startWave, finishWave } from './wave-ledger.js';
...
export async function runAutoforgeLoop(ctx: AutoforgeLoopContext, deps?: Partial<AutoforgeLoopDeps>): Promise<AutoforgeLoopContext> {
  ...
  try {
    ...
    while (!interrupted) {
      ...
      // depth_doctrine WAVE LEDGER (CH-021): emit this cycle as a wave with the canonical schema, so
      // autoforge emits the SAME receipt shape as harden-crusade and ascend (≥3 loops, identical
      // receipts — the bar's central clause). scoreAfter is null here: autoforge measures the next
      // score at the TOP of the following cycle, so a same-cycle after-score would be a guess. The
      // SCHEMA is byte-identical regardless. Best-effort — a ledger write never breaks the loop.
      try {
        const w = await startWave(cwd, { runId: ctx.startedAt, loopName: 'autoforge', waveIndex: ctx.cycleCount, waveType: waveGuard.type, scoreBefore: tracker.overall });
        await finishWave(cwd, w, { status: execRes.shouldBreak ? 'failed' : 'completed', commandsRun: nextCommand ? [nextCommand] : [], decision: execRes.shouldBreak ? 'stop' : 'continue' });
