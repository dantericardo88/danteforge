...
// Per-dimension cycle helpers live in ascend-engine-cycle.ts (file-size split). runAscend drives them.
import {
  executeDimensionCycle, rescoreAndGetDelta, applyStallCorrection,
  runAdversarialCritiqueStep, runPeriodicRetroIfDue, checkConvergenceBreak,
} from './ascend-engine-cycle.js';
...
export interface AscendCycleState {
  cyclesRun: number;
  dimensionsImproved: number;
...
export async function runAscend(options: AscendEngineOptions = {}): Promise<AscendResult> {
  ...
  while (cs.cyclesRun < maxCycles) {
    ...
    // depth_doctrine WAVE LEDGER (CH-021 loop #3): ascend emits the SAME receipt schema as
    // harden-crusade + autoforge, with REAL scoreBefore/After (0-10 matrix scores) — so ≥3 independent
    // loops now drive ONE shared cadence ledger (the bar's central clause). Best-effort.
    try {
      const w = await startWave(cwd, { runId: startedAt, loopName: 'ascend', waveIndex: cs.cyclesRun, waveType: waveGuard.type, dimensionId: nextDim.id, scoreBefore: beforeScore });
      await finishWave(cwd, w, {
        status: buildFailed ? 'failed' : 'completed', scoreAfter: newSelfScore,
        commandsRun: [waveGuard.type === 'depth' ? `validate ${nextDim.id} --preserve-sessions` : `forge ${nextDim.id}`],
        decision: delta > 0.1 ? 'improved' : Math.abs(delta) < 0.1 ? 'stall' : 'continue',
      });
