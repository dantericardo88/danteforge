...
export interface LocalHarvesterFsOps {
  ...
  stat: (targetPath: string) => Promise<{ isDirectory(): boolean; size: number }>;
...
export async function readCodeInsights(
  ...
  for (const manifest of MANIFEST_CANDIDATES) {
    const targetPath = path.join(folderPath, manifest);
    try {
      let content = await fsOps.readFile(targetPath, 'utf-8');
  ...
  for (const entry of ENTRY_POINT_CANDIDATES) {
    ...
    const targetPath = path.join(folderPath, entry);
    try {
      let content = await fsOps.readFile(targetPath, 'utf-8');
...
export async function extractLocalPatterns(
  ...
  const docsText = planningDocs.map((doc) => `### ${doc.name}\n${doc.content}`).join('\n\n');
  const codeText = codeInsights.map((code) => `### ${code.file}\n${code.snippet}`).join('\n\n');
...
export async function synthesizeHarvest(
  ...
  const allPatterns = results.flatMap((result) => result.patterns);
  ...
  const patternSummary = allPatterns
    .map((pattern) => `[${pattern.priority}] ${pattern.category}: ${pattern.name} - ${pattern.description}`)
    .join('\n');

  const sourceLabels = results
    .map((result, index) => result.source.label ?? path.basename(result.source.path) ?? `Source ${index + 1}`)
    .join(', ');
...
QUERY: <query 3>`;
  ...
  try {
    ...
    const synthesis =
      ...
      allPatterns.map((pattern) => pattern.name).join(', ');
    const queries = [...response.matchAll(/QUERY:\s*(.+)/g)].map((match) => match[1]!.trim());
    return { synthesis, recommendedOssQueries: queries };
  } catch {
    return {
      synthesis: allPatterns.map((pattern) => pattern.name).join(', '),
...
export async function harvestLocalSources(
  ...
  const fsOps: LocalHarvesterFsOps = opts._fsOps ?? {
    readFile: (targetPath, encoding) => fs.readFile(targetPath, encoding as BufferEncoding),
  ...
  for (const source of sources) {
    ...
    try {
      ...
      result.tokensUsed = result.planningDocs.reduce((sum, doc) => sum + doc.tokens, 0);
...
export function buildLocalHarvestMarkdown(report: LocalHarvestReport): string {
  ...
  for (const result of report.sources) {
    ...
    if (result.error) {
      ...
      lines.push(`- Planning docs: ${result.planningDocs.map((doc) => doc.name).join(', ') || 'none'}`);
      lines.push(`- Patterns: ${result.patterns.map((pattern) => pattern.name).join(', ') || 'none'}`);
