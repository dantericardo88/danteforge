...
// outcomes to real wired callsites where the seam-free test exercises one; honestly
// downgrades the rest to orphan-pending. This is the repeatable "properly define" step:
// it can never invent evidence, and `validate` afterward produces an honest score.
...
export async function groundOutcomesCommand(options: { project?: string; apply?: boolean; json?: boolean } = {}): Promise<void> {
  ...
  if (options.json) {
    process.stdout.write(JSON.stringify({ project: cwd, counts: summary.counts, dirtyAfter, applied: !!options.apply, results: summary.results }, null, 2) + '\n');
    return;
  }

  logger.info(`Outcome grounding — ${cwd}${options.apply ? '  (APPLIED)' : '  (dry-run; use --apply to write)'}`);
  const c = summary.counts;
  logger.info(`  already-honest ${c['already-honest']} | grounded ${c.grounded} | downgraded ${c.downgraded} | partial ${c.partial}`);
  ...
  if (dirtyAfter.length > 0) {
    logger.warn(`  ⚠ still dirty after grounding (needs manual attention): ${dirtyAfter.join(', ')}`);
    process.exitCode = 1;
  } else {
    logger.success(`  ✓ all ${matrix.dimensions.length} dimensions are honest — run \`danteforge validate --all\` for the honest baseline`);
  }
}
