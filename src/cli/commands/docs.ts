...
const COMMAND_REGISTRY: CommandEntry[] = [
  ...
  { name: 'verify', description: 'Run verification checks on project state & artifacts', options: ['--release', '--live', '--url <url>', '--recompute'], group: 'Pipeline' },
  { name: 'synthesize', description: 'Generate Ultimate Planning Resource (UPR.md) from all artifacts', options: [], group: 'Pipeline' },

  // Automation
  { name: 'spark', args: '[goal]', description: 'Zero-token planning preset for new ideas and project starts', options: ['--prompt'], group: 'Automation' },
  ...
  { name: 'blaze', args: '[goal]', description: 'High-power preset with full party orchestration and verification', options: ['--profile <type>', '--prompt', '--worktree', '--isolation'], group: 'Automation' },
  { name: 'inferno', args: '[goal]', description: 'Maximum-power preset with OSS discovery, full implementation, and evolution', options: ['--profile <type>', '--prompt', '--worktree', '--isolation', '--max-repos <n>'], group: 'Automation' },
