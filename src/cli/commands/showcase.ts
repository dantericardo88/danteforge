...
export interface ShowcaseOptions {
  /** Path to the project to score. Defaults to the bundled demo project (examples/demo-app). */
  project?: string;
...
export async function showcase(options: ShowcaseOptions = {}): Promise<ShowcaseResult> {
  ...
  // Show a relative path in output if possible
  ...
  const lowestDims = DIM_ORDER
    .map(d => ({ dim: d, score: result.displayDimensions[d] ?? 0 }))
...
export function buildCaseStudyMarkdown(
  ...
  for (const dim of DIM_ORDER) {
    ...
    lines.push(`| ${DIM_LABELS[dim]} | ${score.toFixed(1)} / 10 | ${bar} |`);
  }
  ...
  if (result.penalties.length > 0) {
    ...
    for (const p of result.penalties) {
      lines.push(`- **-${p.deduction}** ${p.reason}`);
    }
  }
  ...
  // Improvement opportunities — lowest 5 dimensions
  ...
  for (const { dim, score } of sorted.slice(0, 5)) {
    lines.push(`1. **${DIM_LABELS[dim]}** — currently ${score.toFixed(1)}/10. Run \`danteforge self-improve --focus ${dim}\` to close this gap.`);
  }
