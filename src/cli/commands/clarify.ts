...
export async function clarify(options: {
  ...
  try {
    ...
    const _dnStart = await recordDecision({ session: _dnSess, actorType: 'agent', prompt: 'clarify: refine specification', context: {}, result: 'in-progress', success: false });
...
Output ONLY the markdown content - no preamble.`;
  ...
  try {
    const { getSession, recordDecision } = await import('../../core/decision-node-recorder.js');
    const _dnSess = getSession();
    await recordDecision({ session: _dnSess, parentNodeId: _dnStartNodeId, actorType: 'agent', prompt: 'clarify: refine specification [complete]', result: 'CLARIFY.md written', success: true, latencyMs: Date.now() - _dnT0 });
  } catch { /* best-effort */ }
  });
}
