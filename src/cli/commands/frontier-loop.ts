...
import { runFrontierLoop, type FrontierLoopSeams, type CourtVerdict } from '../../core/frontier-loop.js';
...
export async function runFrontierLoopCli(opts: FrontierLoopCliOptions): Promise<void> {
  ...
  const seams: FrontierLoopSeams = {
    ...
    runCourt: async (dimId) => {
      ...
      } catch (e) { stdout = (e as { stdout?: string }).stdout || ''; } // exits 1 on REJECTED — verdict is in stdout
      const verdict: CourtVerdict = /\bVALIDATED\b/.test(stdout) ? 'VALIDATED'
        : /\bREJECTED\b/.test(stdout) ? 'REJECTED'
          : /independent judges|insufficient/i.test(stdout) ? 'INSUFFICIENT' : 'ERROR';
  ...
  const result = await runFrontierLoop({ dimId: opts.dimId, configPath: opts.config, maxIters: opts.maxIters, cwd }, seams);
