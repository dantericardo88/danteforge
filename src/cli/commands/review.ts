...
async function runReviewLLMMode(
  ctx: ReviewContext,
  ...
  state.projectType = ctx.projectType;
  ...
  try {
    ...
    if (tokens > 120000) {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const chunkPrompt = i === 0 ? chunks[i]! : `Continue the project review. This is part ${i + 1} of ${chunks.length}.\n\n${chunks[i]}`;
        chunkResults.push(await llmFn(chunkPrompt, undefined, { enrichContext: true }));
      }
      stateMd = chunkResults.join('\n\n---\n\n');
      chunkMsg = `${chunks.length} chunks, `;
    } else {
      stateMd = await llmFn(prompt, undefined, { enrichContext: true });
      chunkMsg = '';
    }
    ...
    state.projectType = ctx.projectType;
    state.auditLog.push(`${ctx.timestamp} | review: CURRENT_STATE.md generated via API (${chunkMsg}${ctx.sourceFiles.length} source files)`);
    await saveFn(state);
    await handoff('review', { stateFile: 'CURRENT_STATE.md' });
    logger.success(`CURRENT_STATE.md generated via LLM (${chunkMsg}${ctx.sourceFiles.length} source files reviewed)`);
...
// ── Local mode markdown builder ───────────────────────────────────────────────

function buildLocalReviewMarkdown(ctx: ReviewContext, fullFileTree: string[], isGitRepo: boolean, pkg: { version?: string; dependencies?: Record<string, string>; devDependencies?: Record<string, string> } | null): string {
  const sections: string[] = [];
  sections.push(`# CURRENT_STATE.md`);
  sections.push(`> Generated by DanteForge on ${ctx.timestamp}`);
  sections.push('');
  sections.push('## Project Overview');
  sections.push(`- **Name**: ${ctx.projectName}`);
  if (pkg?.version) sections.push(`- **Version**: ${pkg.version}`);
  sections.push(`- **Detected project type**: \`${ctx.projectType}\``);
  sections.push(`- **Git**: ${isGitRepo ? 'Yes' : 'No'}`);
  sections.push(`- **Working Directory**: ${process.cwd()}`);
  sections.push(`- **Source files**: ${ctx.sourceFiles.length} (of ${fullFileTree.length} total)`);
  sections.push('');
  if (pkg?.dependencies || pkg?.devDependencies) {
    sections.push('## Dependencies');
    if (pkg.dependencies) {
      sections.push('### Runtime');
      for (const [name, version] of Object.entries(pkg.dependencies)) sections.push(`- \`${name}\`: ${version}`);
    }
    if (pkg.devDependencies) {
      sections.push('### Dev');
      for (const [name, version] of Object.entries(pkg.devDependencies)) sections.push(`- \`${name}\`: ${version}`);
    }
    sections.push('');
  }
  if (ctx.recentCommits.length > 0) {
    sections.push('## Recent Changes (Last 10 Commits)');
    sections.push(...ctx.recentCommits);
    sections.push('');
  }
...
export async function review(options: {
  ...
  try {
    ...
    isGitRepo = true;
  } catch { logger.warn('Not a Git repo or no commits yet - skipping Git history'); }
  ...
  if (sourceFiles.length < fullFileTree.length) logger.info(`Filtered: ${fullFileTree.length} total files -> ${sourceFiles.length} source files for review`);

  const ctx: ReviewContext = { projectName, fileTree: sourceFiles, recentCommits, dependencies: pkg?.dependencies ?? null, existingDocs, projectType, sourceFiles, timestamp };

  if (options.prompt) {
    const state = await loadFn();
    state.projectType = projectType;
    ctx.constitution = state.constitution;
    const prompt = buildReviewPrompt({ projectName, constitution: state.constitution, fileTree: sourceFiles, recentCommits, dependencies: pkg?.dependencies ?? null, existingDocs });
    const tokens = estimateTokens(prompt);
    logger.info(`Estimated tokens: ~${tokens.toLocaleString()}`);
    const savedPath = await savePrompt('review', prompt);
    displayPrompt(prompt, `Steps:\n1. Copy the prompt above\n2. Paste into Claude Code, ChatGPT, or any LLM\n3. Copy the generated markdown output\n4. Save it as .danteforge/CURRENT_STATE.md (or run: danteforge import <file> --as CURRENT_STATE.md)\n5. Run "danteforge constitution" to establish project principles\n6. Run "danteforge specify <goal>" to continue the pipeline\n\nPrompt also saved to: ${savedPath}`);
  ...
  if (llmAvailable) {
    const state = await loadFn();
    ctx.constitution = state.constitution;
    const succeeded = await runReviewLLMMode(ctx, llmFn, loadFn, saveFn);
    if (succeeded) return;
  }

  // Local mode: build CURRENT_STATE.md from raw data (no LLM)
  const stateMd = buildLocalReviewMarkdown(ctx, fullFileTree, isGitRepo, pkg);
  await fs.mkdir(STATE_DIR, { recursive: true });
  await fs.writeFile(path.join(STATE_DIR, 'CURRENT_STATE.md'), stateMd);
  const state = await loadFn();
  resetExecutionStateForReview(state);
