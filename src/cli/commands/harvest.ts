...
export async function harvest(
  ...
  const displayPromptFn = options._displayPrompt ?? displayPrompt;
  const llmCaller = options._llmCaller ?? ((p: string) => callLLM(p));
  ...
  const extractJsonFn = options._extractJson ?? extractJson;
  ...
  if (llmAvailable) {
    ...
    try {
      ...
      const step1Raw = await llmCaller(buildStep1Prompt(system, mode));
      const step1Data = extractJsonFn<HarvestTrack['step1Discovery']>(step1Raw);
      ...
      const step2Raw = await llmCaller(buildStep2Prompt(system, track.step1Discovery));
      const step2Data = extractJsonFn<HarvestTrack['step2Constitution']>(step2Raw);
      track.step2Constitution = step2Data;
      logger.success('Step 2 complete');

      // Step 3: Wiring
      logger.info('Step 3: Wiring...');
      const step3Raw = await llmCaller(buildStep3Prompt(system, track.step1Discovery));
      const step3Data = extractJsonFn<HarvestTrack['step3Wiring']>(step3Raw);
      ...
      if (mode === 'full') {
        ...
        const step4Raw = await llmCaller(buildStep4Prompt(system, track.step1Discovery));
        const step4Data = extractJsonFn<NonNullable<HarvestTrack['step4Evidence']>>(step4Raw);
      ...
      const step5Raw = await llmCaller(buildStep5Prompt(system, track));
      const step5Data = extractJsonFn<Omit<HarvestTrack['step5Ratification'], 'hash'>>(step5Raw);
