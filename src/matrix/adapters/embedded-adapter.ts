...
function buildInstructions(lease: AgentLease): string {
  return [
    ...
    `2. Make the changes inline using your own Edit/Write tools, **inside the worktree at \`${lease.worktreePath}\`** — NOT in the main repo checkout. Every `,
    `   ownedPath is relative to that worktree root. The worktree is on branch \`${lease.branch}\`; edits there don't touch your main branch.`,
    '3. STAY within `ownedPaths`. Never touch `forbiddenPaths`.',
    '4. Run any required local checks (typecheck/tests) **from the worktree directory** before reporting completion.',
    `5. When done, run: \`danteforge matrix-kernel embedded-complete ${lease.id}\``,
    '   That command captures your diff via `git diff --name-only HEAD` inside the worktree and feeds it into verify-court.',
    '',
